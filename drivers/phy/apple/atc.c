...
struct atcphy_dp_ops {
	...
	int (*set_rate)(struct apple_atcphy *atcphy, unsigned int rate);
 ...
 * @dp_link_rate: DisplayPort link rate; T8122 uses Mbps, -1 for idle and 0 for
 *               incomplete setup/shutdown that still needs cleanup
...
static int atcphy_dp_start_link_t8122(struct apple_atcphy *atcphy, u8 pmas)
{
	...
	if (!pmas || pmas > 3)
		return -EINVAL;
	ret = atcphy_dp_start_pll_t8122(atcphy, &dp_hbr3_config_t8122);
	if (ret)
		return ret;
	...
	for (unsigned int lane = 0; lane < 2; lane++) {
		if (!(pmas & BIT(lane)))
			continue;
		atcphy_dp_configure_tx_t8122(atcphy, lane);
		atcphy_dp_configure_rxtx_t8122(atcphy, lane);
	}
...
static int atcphy_dp_stop_link_t8122(struct apple_atcphy *atcphy)
{
	...
	lockdep_assert_held(&atcphy->lock);
	if (atcphy->dp_link_rate == -1)
		return 0;
	cfg = atcphy_get_mode_config(atcphy, atcphy->mode);
	atcphy->dp_link_rate = 0;
...
static int atcphy_dp_set_rate_t8122(struct apple_atcphy *atcphy, unsigned int rate)
{
	const struct atcphy_mode_configuration *cfg;
	u8 pmas;
	int ret;
	...
	if (!rate)
		return atcphy_dp_stop_link_t8122(atcphy);
	if (rate != 8100)
		return -EOPNOTSUPP;
	...
	if (!pmas)
		return -EINVAL;
	if (atcphy->dp_link_rate == rate)
		return 0;
	ret = atcphy_dp_stop_link_t8122(atcphy);
	if (ret)
		return ret;
	atcphy->dp_link_rate = 0;
	ret = atcphy_dp_start_link_t8122(atcphy, pmas);
	if (ret) {
		atcphy_dp_stop_link_t8122(atcphy);
		return ret;
	}
	atcphy->dp_link_rate = rate;
	return 0;
}
...
static int atcphy_dpphy_configure(struct phy *phy, union phy_configure_opts *opts_)
{
	...
	if (opts->set_rate) {
		...
		if (atcphy->hw->dp->set_rate)
			return atcphy->hw->dp->set_rate(atcphy, opts->link_rate);
...
static const struct of_device_id atcphy_match[] = {
	{ .compatible = "apple,t6030-atcphy", .data = &atcphy_hw_t6030 },
